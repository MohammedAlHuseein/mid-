...
function AgencyDashboard() {
  ...
  const totalUsers = (JSON.parse(localStorage.getItem("all_users")) || [])
    .length;
  ...
  const handleSaveEdit = () => {
    const updated = agencies.map((a) =>
      a.email === editEmail ? editFormData : a,
    );
    setagencies(updated);
    localStorage.setItem("all_agency", JSON.stringify(updated));
    setEditEmail(null);
      ...
      <Box sx={{ flexGrow: 1 }}>
        <AppBar position="static" sx={{ backgroundColor: "rgb(12, 12, 12)" }}>
          <Toolbar>
            ...
            <Button color="inherit" onClick={hanleLogout}>
              Logout
            </Button>
          </Toolbar>
        </AppBar>
        ...
        <Box sx={{ p: 3 }}>
                        ...
                        <TableCell>
                          <Box sx={{ display: "flex", gap: 1 }}>
                            ...
                            <Button
                              ...
                              color="error"
                              size="small"
                              onClick={() => handleDelete(agency.email)}
                            >
                              Delete
                            </Button>
                          </Box>
          ...
          <Typography
            variant="h5"
            sx={{ mt: 5, mb: 2, fontWeight: "bold", color: "#e64a19" }}
          >
            Customer Orders
          </Typography>

          <TableContainer component={Paper} sx={{ boxShadow: 3, mb: 5 }}>
            <Table>
              <TableHead sx={{ backgroundColor: "#e64a19" }}>
                <TableRow>
                  ...
                  orders.map((order) => (
                    <TableRow key={order.id}>
                      ...
                      <TableCell
                        sx={{
                          color:
                            order.status === "Done" ? "#2e7d32" : "#f57c00",
                          fontWeight: "bold",
                        }}
                      >
                        {order.status}
